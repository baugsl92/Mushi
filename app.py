...
def configured_repository_slug() -> str | None:
    """Use a Streamlit secret when configured, then fall back to GitHub/git detection."""
    try:
        secret_value = st.secrets.get("MUSHROOM_WATCH_REPOSITORY")
    except (FileNotFoundError, KeyError):
        secret_value = None
    return normalize_repository_slug(str(secret_value)) if secret_value else detect_repository_slug(ROOT)


DETECTED_REPOSITORY = configured_repository_slug()
...
if not analysis:
    ...
    st.markdown(
        ...
        - County/state selection, area-balanced radius points, GitHub Issue field logs, ntfy, Streamlit Community Cloud, tests, and GitHub Actions
        """
    )
...
with field_tab:
    ...
    st.info(
        ...
        "This survives Streamlit restarts and keeps the calibration record with the repository."
    )
    repository_input = st.text_input(
        "GitHub repository",
        value=DETECTED_REPOSITORY or "",
        placeholder="owner/repository",
        help="Set this once in Streamlit secrets, or enter owner/repository here.",
    ...
    with st.expander("Temporary scratch log inside this running app"):
        st.warning(
            "Scratch entries use a local SQLite file and may disappear when the hosted Streamlit app restarts. "
